<script type='text/javascript'>
	    ...
	    progressbox.width(fieldwidth);
			...
			beforeSend: function() { //actions taking place before sending the form
			    ...
			    $('#title-error').html('');
			    $('#title-error').removeClass('alert alert-error');
			    $('#description-error').html('');
			    $('#description-error').removeClass('alert alert-error');
			    $('#datepicker-error').html('');
			    $('#datepicker-error').removeClass('alert alert-error');
			    $('#from-error').html('');
			    $('#from-error').removeClass('alert alert-error');
			    $('#to-error').html('');
			    $('#to-error').removeClass('alert alert-error');
			    $('#itemfile-error').html('');
			    $('#itemfile-error').removeClass('alert alert-error');
			    $('#debug_area').html('');
			    $('#debug_area').removeClass('alert alert-error');
			},
			...
			complete: function(response) { // on complete START                       
				...
				if (obj[0] == 'false') {
					...
					// If file and database record were stored, dipslay a success message and reload page after 2 seconds
					$("#uploadForm").after("<div class='alert alert-success' style='margin-top:5px'>The file was successfully uploaded!</div>");
					setTimeout(function(){
